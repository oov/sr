...
enum spng__internal { SPNG__IO_SIGNAL = 1 << 9, SPNG__CTX_FLAGS_ALL = (SPNG_CTX_IGNORE_ADLER32 | SPNG_CTX_ENCODER) };

#define SPNG_STR(x) _SPNG_STR(x)
#define _SPNG_STR(x) #x

#define SPNG_VERSION_STRING SPNG_STR(SPNG_VERSION_MAJOR) "." SPNG_STR(SPNG_VERSION_MINOR) "." SPNG_STR(SPNG_VERSION_PATCH)
...
*/
static int defilter_scanline(
    const unsigned char *prev_scanline, unsigned char *scanline, size_t scanline_width, unsigned bytes_per_pixel, unsigned filter) {
...
static int32_t filter_sum(
    const unsigned char *prev_scanline, const unsigned char *scanline, size_t size, unsigned bytes_per_pixel, const unsigned filter) {
...
static unsigned get_best_filter(
    const unsigned char *prev_scanline, const unsigned char *scanline, size_t scanline_width, unsigned bytes_per_pixel, const int choices) {
  if (!choices)
...
static void
expand_row(unsigned char *row, const unsigned char *scanline, const union spng__decode_plte *decode_plte, uint32_t width, int fmt) {
  uint32_t i = 0;
  unsigned char *px;
  unsigned char entry;
...
static int check_chrm_int(const struct spng_chrm_int *chrm_int) {
  ...
  if (chrm_int->white_point_x > spng_u32max || chrm_int->white_point_y > spng_u32max || chrm_int->red_x > spng_u32max ||
      chrm_int->red_y > spng_u32max || chrm_int->green_x > spng_u32max || chrm_int->green_y > spng_u32max ||
    ...
    } else /* Arbitrary-length chunk */
    {
      ...
      if (!memcmp(chunk.type, type_exif, 4)) {
        ...
        ret = spng__inflate_stream(
            ctx, &ctx->iccp.profile, &ctx->iccp.profile_len, 0, ctx->data + keyword_len + 2, peek_bytes - (keyword_len + 2));

        if (ret)
          return ret;

        ctx->stored.iccp = 1;
      } else if (!memcmp(chunk.type, type_text, 4) || !memcmp(chunk.type, type_ztxt, 4) || !memcmp(chunk.type, type_itxt, 4)) {
        if (!chunk.length)
          return SPNG_ECHUNK_SIZE;
        ...
        if (increase_cache_usage(ctx, chunk.length + sizeof(struct spng_splt), 1))
          return SPNG_ECHUNK_LIMITS;
...
int spng_decode_image(spng_ctx *ctx, void *out, size_t len, int fmt, int flags) {
  ...
  if (ihdr->bit_depth == 16 && fmt & (SPNG_FMT_RGBA8 | SPNG_FMT_RGB8)) { /* samples are scaled down by 8 bits in the decode loop */
    sb->red_bits -= 8;
    sb->green_bits -= 8;
    sb->blue_bits -= 8;
    sb->alpha_bits -= 8;
...
#if defined(SPNG_ARM)
      if (fmt == SPNG_FMT_RGB8 && ihdr->bit_depth == 8) { /* Working with 3 bytes at a time is more of an ARM thing */
...
spng_ctx *spng_ctx_new(int flags) {
  struct spng_alloc alloc = {.malloc_fn = malloc, .realloc_fn = realloc, .calloc_fn = calloc, .free_fn = free};
...
spng_ctx *spng_ctx_new2(struct spng_alloc *alloc, int flags) {
  ...
  const struct spng__zlib_options image_defaults = {
      .compression_level = Z_DEFAULT_COMPRESSION, .window_bits = 15, .mem_level = 8, .strategy = Z_FILTERED, .data_type = 0 /* Z_BINARY */
  };
