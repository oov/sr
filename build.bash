...
mkdir -p build/tools
. src/3rd/ovbase/setup-llvm-mingw.bash --dir $PWD/build/tools
...
for arch in x86_64; do
  ...
  if [ "${REBUILD}" -eq 1 ] || [ ! -e "${destdir}/CMakeCache.txt" ]; then
    ...
    cmake -S .. -B "${destdir}" --preset default \
      ...
      -DCMAKE_TOOLCHAIN_FILE="src/3rd/ovbase/cmake/llvm-mingw.cmake" \
...
mkdir -p "${destdir}/bin"
cp -r "${destdir}/x86_64/bin/"* "${destdir}/bin"
